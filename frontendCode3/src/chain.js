...
// Only the five functions this app calls.
const ABI = [
...
/** A contract bound to the node with no wallet - enough for view calls. */
...
/**
 * Read-only check, costs no gas and needs no wallet at all - a view call is
 * never signed, so this works with or without MetaMask installed.
 */
export async function hasConsent(granterWallet, receiverWallet) {
  return readOnlyContract().hasConsent(granterWallet, receiverWallet);
}
...
export async function verifyReview(text) {
  return readOnlyContract().verifyReview(hashReview(text));
}
