...
/**
 * Asking to see someone's photo and contact details, and their decision on it.
 *
 ...
 * owner's own key signing grantConsent. See ConsentController.contact and
 * PhotoController - both read the contract, never this table.
 */
@RestController
...
public class RequestController {
    ...
    public RequestController(ContactRequestRepo requests, UserRepo users, ProfileRepo profiles) {
        this.requests = requests;
        this.users = users;
        this.profiles = profiles;
    }

    /** Ask a user for their details. Asking twice reuses the same row. */
    @PostMapping
    ...
    public List<Map<String, Object>> incoming(@PathVariable Long ownerId) {
        ...
        for (ContactRequest r : requests.findByOwnerIdAndStatusOrderByCreatedAtDesc(
            ...
            row.put("requesterVerified", Boolean.TRUE.equals(requester.get().verified));
            profiles.findByUserId(r.requesterId).ifPresent(p -> {
                row.put("requesterArea", p.area);
                row.put("requesterCity", p.city);
            });
     ...
     *
     * This does not release anything by itself. Contact details and the photo
     * are still gated on hasConsent in the contract, so if the grant
     * transaction never reached the chain this row is a decision with no effect.
     */
