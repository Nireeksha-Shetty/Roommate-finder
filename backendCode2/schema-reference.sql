...
CREATE TABLE users (
    ...
    photo_path      VARCHAR(255),          -- random UUID filename on disk, never a URL
...
CREATE TABLE profiles (
    ...
    city               VARCHAR(255),
    area               VARCHAR(255),
    budget             INT,                -- rupees per month, their own share
    gender             VARCHAR(16),        -- male | female | other
    gender_preference  VARCHAR(8),         -- same | any, applied mutually
    sharing            VARCHAR(32),        -- Single occupancy | Double sharing | Triple sharing
